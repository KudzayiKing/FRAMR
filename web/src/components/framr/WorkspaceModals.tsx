...
export function ProductModal({ open, onClose, onSave }: { open: boolean; onClose: () => void; onSave: (asset: ProductAsset) => void }) {
  ...
  return <Modal open={open} onClose={onClose} title="Add a product asset" className="max-w-md"><div className="px-7 pb-7"><p className="mt-1 text-xs text-inksoft">Products are reusable commercial assets — across placements, versions and campaigns.</p><button onClick={() => setPreview(true)} className="frame-grow relative mt-5 flex w-full flex-col items-center gap-2 rounded-lg border-2 border-dashed border-line bg-paper2/60 p-6 text-inksoft transition hover:bg-paper2"><FrameCorners className="text-accent" /><Image size={24} /><span className="text-xs font-semibold">Product image</span></button>{preview && <img src={IMG.espressoProduct} alt="Product preview" className="mt-3 h-24 w-full rounded-lg border border-line object-cover" />}<div className="mt-4 grid grid-cols-2 gap-3"><label className="text-xs font-semibold">Product name<input value={name} onChange={(event) => setName(event.target.value)} className="mt-1 h-10 w-full rounded-md border border-line bg-white px-3 text-sm font-normal" placeholder="Model A rice cooker" /></label><label className="text-xs font-semibold">Brand<input value={brand} onChange={(event) => setBrand(event.target.value)} className="mt-1 h-10 w-full rounded-md border border-line bg-white px-3 text-sm font-normal" placeholder="Auris" /></label></div><label className="mt-3 block text-xs font-semibold">Description <span className="font-normal text-inksoft">(optional)</span><textarea className="mt-1 h-16 w-full rounded-md border border-line bg-white px-3 py-2 text-sm font-normal" placeholder="Induction rice cooker with ceramic core…" /></label><FramrButton className="mt-5 w-full" onClick={save}>Save product asset</FramrButton></div></Modal>;
}
...
export function GenerationModal({ open, onClose, video, onFinish }: { open: boolean; onClose: () => void; video: Video | null; onFinish: () => void }) {
  ...
  useEffect(() => { if (!open) return; const timer = window.setTimeout(() => setDone(true), 2600); return () => window.clearTimeout(timer); }, [open]);
  if (!video) return null;
  const finish = () => { onFinish(); onClose(); toast("Version created — original untouched."); };
  return <Modal open={open} onClose={onClose} className="max-w-xl"><div className="grid sm:grid-cols-[220px_1fr]"><div className="relative aspect-[9/12] overflow-hidden bg-night sm:aspect-auto"><img src={video.thumbnail} alt="Placement source frame" className="absolute inset-0 h-full w-full object-cover opacity-90" />{!done && <div className="scanline" />}<FrameCorners className="m-3 text-white/70" /><Chip className="absolute bottom-3 left-3 bg-night/80 text-white">{done ? <Check size={12} /> : <span className="rec-dot" />}{done ? "Complete" : "Generating"}</Chip></div><div className="p-7"><div className="text-[11px] font-bold uppercase tracking-[.14em] text-accent">GenerationJob</div><h2 className="mt-1 text-xl font-extrabold tracking-tight">Creating your placement</h2><p className="mt-1 text-xs text-inksoft">The original is never modified. A new version branch is created.</p><div className="mt-5 space-y-3">{["Queued", "Analyzing scene & tracking movement", "Mapping product & preserving lighting", "Generating frames", "Finalizing version"].map((step, index) => <div className="flex items-center gap-3 text-sm" key={step}><span className="flex h-6 w-6 items-center justify-center rounded-full border border-line bg-paper2">{done || index === 0 ? <Check size={14} className="text-emerald-600" /> : <Loader2 size={14} className="animate-spin text-accent" />}</span><span className={done || index === 0 ? "" : "text-inksoft"}>{step}</span></div>)}</div>{done && <div className="mt-5 rounded-lg border border-line bg-paper2/60 p-4"><div className="flex items-center gap-2 text-sm font-bold"><span className="flex h-5 w-5 items-center justify-center rounded-full bg-ink text-paper"><Check size={12} /></span>Version created</div><div className="mt-2 font-mono text-[11px] text-inksoft">1080×1920 · source audio preserved · render complete</div><FramrButton className="mt-4 w-full" onClick={finish}>View versions</FramrButton></div>}</div></div></Modal>;
}
